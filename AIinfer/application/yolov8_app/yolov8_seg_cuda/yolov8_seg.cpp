...
namespace tensorrt_infer
{
    namespace yolov8_cuda
    {
        ...
        SegBoxArray YOLOv8Seg::forward(const Image &image,vector<float> &mt1,vector<float> &mt2,vector<float> &mt3)
        {
            auto output = forwards({image},mt1,mt2,mt3);
            if (output.empty())
                return {};
            return output[0];
        }

        BatchSegBoxArray YOLOv8Seg::forwards(const std::vector<Image> &images,vector<float> &mt1,vector<float> &mt2,vector<float> &mt3)
        {
            ...
            cudaEventElapsedTime(&tc1, t1, t2);
            ...
            cudaEventElapsedTime(&tc2, t2, t3);
            ...
            cudaEventElapsedTime(&tc3, t3, t4);

            // printf(" %f ms pre,%f ms infer,%f ms post, \n", tc1, tc2, tc3);
            mt1.push_back(tc1);
            mt2.push_back(tc2);
            mt3.push_back(tc3);
