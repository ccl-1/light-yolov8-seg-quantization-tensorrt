...
namespace tensorrt_infer
{
    namespace yolov8_cuda
    {
        ...
        public:
            ...
            // forward
            SegBoxArray forward(const Image &image, vector<float> &mt1,vector<float> &mt2,vector<float> &mt3);
            BatchSegBoxArray forwards(const std::vector<Image> &images, vector<float> &mt1,vector<float> &mt2,vector<float> &mt3);
