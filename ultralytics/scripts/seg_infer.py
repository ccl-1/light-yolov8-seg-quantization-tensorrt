...
def plot_result(det_cpu, dst_img, masks, category_names, save_path):
    for i, item in enumerate(det_cpu):
        ...
        dst_img[box_y1:box_y2, box_x1:box_x2] = cv2.addWeighted(bbox_image, 1, mask_colored, 0.4, 0)

    res = Image.fromarray(dst_img)
    # res.save(save_path)
    print(save_path)
    plt.imshow(dst_img)
    plt.show()
...
def test_yolov8_segment():
    parser = argparse.ArgumentParser()
    # parser.add_argument('--data', default='assets/imgs/rs05202.jpg', type=str, help='Images file')
    parser.add_argument('--data', default='assets/imgs', type=str, help='Images file')
    # parser.add_argument('--data', default='assets/imgs/rs05591.jpg', type=str, help='Images file')

    # parser.add_argument('--weight', default='./runs/segment/yolov8n-seg/weights/best.pt', type=str)
    parser.add_argument('--weight', default='./runs/segment/yolov8n-seg-ours/weights/best.pt', type=str)
    parser.add_argument('--imgsz', default=[640, 640], type=list, help='Images size')
    # parser.add_argument('--imgsz', default=[480, 640], type=list, help='Images size')
    ...
    img_name = sorted(os.listdir(args.data))
    for i in img_name:
        img_path = os.path.join(args.data, i)
        save_path = img_path.replace("imgs", "out")
        t0 = time.time()
        ori_img = cv2.imread(img_path)
        # ori_img = cv2.imread(args.data)

        t1 = time.time()

        img = data_preprocess(model, ori_img, args.imgsz)
        t2 = time.time()


        # warm-up
        for i in range(0, number_of_warmup_runs):
        ...
        preds = result[0]
        proto = result[1][-1]
        det = non_max_suppression(preds, conf_thres=0.25, iou_thres=0.45, nc=len(model.CLASSES))[0] # NMS
        det[:, :4] = scale_boxes(img.shape[2:], det[:, :4], ori_img.shape)                        # bbox还原至原图尺寸
        masks = process_mask(proto[0], det[:, 6:], det[:, :4], img.shape[2:], ori_img.shape[0:2]) # mask转换成原图尺寸并做裁剪
        category_names = model.CLASSES
        t5 = time.time()

        tc0 = (t1-t0)*1000
        tc1 = (t2-t1)*1000
        tc2 = (t4-t3)*1000./loop_count
        tc3 = (t5-t4)*1000

        total = tc0+tc1+tc2+tc3
        print("%.2f ms imread, %.2lf ms pre,  %.2f ms infer,  %.2f ms post,  %.2f ms total \n"%(tc0,tc1,tc2,tc3,total))

        # 画图
        plot_result(det.cpu().numpy(), ori_img, masks, category_names,save_path)
