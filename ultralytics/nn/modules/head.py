...
__all__ = 'Detect', 'Segment', 'Pose', 'Classify', 'RTDETRDecoder', \
      'Detect_Efficient', 'Segment_Efficient', 'DetectAux','Segment_light', 'Detect_light'
...
class Detect_light(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 1)) for x in ch) # one 1x1 Conv, one 3x3 Conv
        
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(nn.Sequential(Conv(x, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)

        # lighthead-group
        # self.cv2 = nn.ModuleList(nn.Sequential(Conv(x, c2, 3, g=x // 16), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        # self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3, g=x // 16), nn.Conv2d(c3, self.nc, 1)) for x in ch)
...
class Segment_light(Detect_light):
    """YOLOv8 Segment head for segmentation models."""
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        ...
        self.npr = npr  # number of protos
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = Detect_light.forward

        c4 = max(ch[0] // 4, self.nm)
        # self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 1), Conv(c4, c4, 3, g=x // 16), nn.Conv2d(c4, self.nm, 1)) for x in ch)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 1), Conv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1)) for x in ch)
        # self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, x, 1), Conv(x, c4, 3), nn.Conv2d(c4, self.nm, 1)) for x in ch)
...
class Segment_Efficient(Detect_Efficient):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        ...
        c4 = max(ch[0] // 4, self.nm)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 1), Conv(c4, c4, 1), nn.Conv2d(c4, self.nm, 1)) for x in ch)
