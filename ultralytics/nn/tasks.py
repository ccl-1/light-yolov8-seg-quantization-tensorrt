...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    PConv, C3_Faster, C2f_Faster, C3_Faster_EMA, C2f_Faster_EMA,
                                    Classify, Concat, Conv, Conv2, ConvTranspose, Detect, DWConv, DWConvTranspose2d,
                                    Focus, GhostBottleneck, GhostConv, HGBlock, HGStem, Pose, RepC3, RepConv,
                                    RTDETRDecoder, Segment, Detect_Efficient, Segment_Efficient,Segment_light, Detect_light)
from ultralytics.utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, colorstr, emojis, yaml_load
from ultralytics.utils.checks import check_requirements, check_suffix, check_yaml
from ultralytics.utils.loss import v8ClassificationLoss, v8DetectionLoss, v8PoseLoss, v8SegmentationLoss, v8SemanticLoss
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment, Detect_Efficient, Segment_Efficient, Segment_light, Detect_light)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment, Detect_Efficient, Segment_Efficient, Segment_light, Detect_light,Pose)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
           
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Segment_Efficient, Segment_light,Pose)) else self.forward(x)
...
class SemanticModel(DetectionModel):
    """YOLOv8 Semantic segmentation model."""

    def __init__(self, cfg='yolov8n-seg.yaml', ch=3, nc=None, verbose=True):
        """Initialize YOLOv8 Semantic segmentation model with given config and parameters."""
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)

    def init_criterion(self):
        # return v8SegmentationLoss(self)
        return v8SemanticLoss(self) 
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        ...
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, 
                 Detect, Segment, Detect_Efficient, Segment_Efficient, Segment_light, Detect_light):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in (Detect, Segment, Pose, RTDETRDecoder, Detect_Efficient, Segment_Efficient, Segment_light, Detect_light):
            args.append([ch[x] for x in f])
            if m in (Segment, Segment_Efficient, Segment_light):
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in ('classify', 'classifier', 'cls', 'fc'):
            return 'classify'
        if m in ('detect', 'Detect_Efficient', 'Detect_light'):
            return 'detect'
        if m in ('segment', 'Segment_Efficient', 'Segment_light'):
            return 'segment'
        if m == 'pose':
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect, Detect_Efficient, Detect_light):
                return 'detect'
            elif isinstance(m, Segment, Segment_Efficient, Segment_light):
                return 'segment'
